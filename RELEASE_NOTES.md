...
### Learning-system hardening and dual-track convergence

- General English is now the default learning track for fresh installs;
  IELTS Academic remains a fully registered optional Domain Pack with its
  own sessions, rubrics and capabilities (schema v35);
- rich word cards: offline bundled presets for the ~2900 high-frequency
  words (IPA, part of speech, English/Chinese definitions and inflections
  from open ECDICT data, generated by scripts/build_vocab_presets.py) plus
  local lemminflect-derived word forms for every word;
- on-demand model enrichment: POST /api/v1/vocabulary/{id}/enrich fills
  definitions, examples, synonyms and antonyms through the configured
  provider route and stores them with provenance;
- adaptive spaced review: recalled words climb a 1-2-4-7-14-30-60 day
  ladder; missed words reset to the next day (schema v35 success_streak);
- automatic backups: the local service creates one when the newest backup
  is over a week old and keeps the five most recent automatic backups;
- tutor planning degrades gracefully: a failed planner or a hallucinated
  tool call falls back to one direct answer instead of failing the turn,
  and oversized tool results keep both head and tail;
- Skills load only the reference documents the current teaching phase
  needs, shrinking planning prompts while preserving full traceability;
