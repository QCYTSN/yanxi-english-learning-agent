...
Status: V0.7 companion UI implemented on 2026-07-22. Final visual style is not
...
```powershell
ielts-coach ui start --port 8765
ielts-coach ui start --no-open
ielts-coach ui start --home D:\IELTS_AI\data
```

The server always binds to `127.0.0.1`. A one-time launch token is exchanged
for a SameSite HttpOnly browser session and is removed from the address bar.

## Implemented V0.7 scope

- React + TypeScript + Vite application packaged inside the Python wheel;
- Today, Practice, Writing, Reading, Feedback, Library, History/Progress and
  Settings routes;
...
- Task 1 structured tables, registered PNG/JPEG/WebP media and original-image
  viewing;
- schema v6 infrastructure for drafts, idempotency, media and Agent runs;
...
- Speaking workspace, audio capture or pronunciation scoring;
