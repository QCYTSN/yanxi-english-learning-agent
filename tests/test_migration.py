...
def test_v01_database_is_migrated_without_data_loss(tmp_path: Path):
    ...
    assert backups[0]["kind"] == "pre-migration-legacy-to-13"
    assert verify_backup(home, backups[0]["backup_id"])["valid"] is True


@pytest.mark.parametrize("version", ["1", "5", "9"])
def test_historical_version_markers_migrate_to_v13(version: str, tmp_path: Path):
    ...
    backups = list_backups(home)
    assert backups[0]["kind"] == f"pre-migration-{version}-to-13"
    assert verify_backup(home, backups[0]["backup_id"])["valid"] is True
...
def test_interrupted_migration_creates_recoverable_snapshot(
    ...
    assert backup["kind"] == "pre-migration-legacy-to-13"
